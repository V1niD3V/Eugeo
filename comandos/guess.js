...
module.exports.run = async (bot, message, args) => {
    if (serversmap.has(message.guild.id)) {
        return message.channel.send(new Discord.RichEmbed()
            .setTitle('Erro')
            .setDescription('Já existe uma partida do jogo rodando neste servidor.')
            .setColor("#FF0000"));
    }
    ...
    message.channel.send(new Discord.RichEmbed()
        ...
        .setDescription(`Seu número secreto está entre **${gameconfig.minimum}** e **${gameconfig.maximum}.**
    ...
    game_collector.on('collect', u_msg => {
        ...
        if (parseInt(u_msg.content) === random_number) {
            if (gameconfig.trycount <= 1) gameconfig.title = 'Cagado';
            else if (gameconfig.trycount <= 2) gameconfig.title = 'Deus';
            else if (gameconfig.trycount <= 4) gameconfig.title = 'Calculista';
    ...
    game_collector.on('end', (msg, reason) => {
        ...
        switch (reason) {
            ...
            case 'win':
                return message.channel.send(new Discord.RichEmbed()
                    .setTitle('Vitória')
                    .setDescription('Parabéns, você acertou o número secreto!')
            ...
            default:
                return message.channel.send(new Discord.RichEmbed()
                    .setTitle('Game Over!')
                    .setDescription(`O jogo excedeu o limite de 5 minutos.
                    O número secreto era **${random_number}**.`)
                    .setColor('#FF0000'));
        }
    })
